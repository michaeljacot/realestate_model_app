...
MAPS_DIR = Path(__file__).resolve().parent / "property_maps"
...
def _remove_map_if_exists(path: Optional[str]) -> None:
    if not path:
        return
    try:
        existing = Path(path)
...
def create_property_map_image(
    address: Optional[str],
    latitude: Optional[float],
    longitude: Optional[float],
    existing_path: Optional[str] = None,
) -> Optional[str]:
    """Generate a simple static map preview highlighting the property location."""

    if latitude is None or longitude is None:
        _remove_map_if_exists(existing_path)
        return None

    MAPS_DIR.mkdir(parents=True, exist_ok=True)
    filename = f"property_{uuid.uuid4().hex}.png"
    output_path = MAPS_DIR / filename

    fig, ax = plt.subplots(figsize=(4, 4))
    lat_range = max(abs(latitude) * 0.01, 0.02)
    lon_range = max(abs(longitude) * 0.01, 0.02)
    ax.set_xlim(longitude - lon_range, longitude + lon_range)
    ax.set_ylim(latitude - lat_range, latitude + lat_range)
    ax.scatter([longitude], [latitude], color="#d62728", s=160, marker="*", edgecolor="black", linewidths=1.5)
    title = address if address else "Property Location"
...
def render_simulator_page():
    ...
    with c_left:
        ...
        with st.form("prop_form"):
            ...
            if submitted_prop:
                lat_val = lat if lat != 0.0 else None
                lon_val = lon if lon != 0.0 else None
                map_path = create_property_map_image(address or None, lat_val, lon_val, prop_data.get("map_image_path"))
                up = {
                    "id": prop_data.get("id"),
        ...
        if st.session_state.selected_property_id:
            with st.expander("Danger zone"):
                if st.button("Delete property"):
                    _remove_map_if_exists(prop_data.get("map_image_path"))
...
def render_dashboard_page():
    ...
    summaries = sim_db.list_properties_with_latest_run(DB_PATH)

    if not summaries:
        st.info("Run at least one simulation to populate the dashboard.")
        return
    ...
    for idx, record in enumerate(summaries):
        ...
        with col:
            with st.container(border=True):
                map_path = record.get("map_image_path")
                if map_path and Path(map_path).exists():
                    st.image(map_path, use_column_width=True)
                else:
                    st.write("No map available")

                address = record.get("address") or f"Property #{record['id']}"
                st.subheader(address)
                st.caption(f"Last run: {record.get('run_at') or 'N/A'}")
                if record.get("scenario_name"):
                    st.caption(f"Scenario: {record['scenario_name']}")

                m_col1, m_col2, m_col3 = st.columns(3)
                with m_col1:
                    st.metric("Monthly Cash Flow", f"${record.get('ending_monthly_cf', 0):,.0f}")
                    st.metric("Monthly Mortgage", f"${record.get('monthly_mortgage', 0):,.0f}")
                with m_col2:
                    st.metric("Initial CoC (%)", f"{record.get('initial_coc', 0):.1f}%")
                    st.metric("Total Return ($)", f"${record.get('total_return_est', 0):,.0f}")
                with m_col3:
                    payback = record.get("payback_month")
                    payback_txt = f"{payback} mo" if payback and payback > 0 else "N/A"
                    st.metric("Payback", payback_txt)
                    st.metric("Terminal Equity", f"${record.get('terminal_equity', 0):,.0f}")

                if st.button("Open Simulation", key=f"open_{record['id']}"):
                    st.session_state.selected_property_id = record["id"]
                    st.session_state.selected_scenario_id = record.get("scenario_id")
...
page_choice = st.sidebar.radio(
    "Navigation",
    ["Simulator", "Dashboard"],
    index=0 if st.session_state.active_page == "Simulator" else 1,
)
st.session_state.active_page = page_choice

if st.session_state.active_page == "Simulator":
